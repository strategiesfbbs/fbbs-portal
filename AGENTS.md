...
- `server/fred-series.js` — FRED rate benchmarks (2026-06-11): SOFR / effective fed funds / 10Y breakeven, 6h TTL, per-series outage keeps that series' cached print. **Dormant until `FRED_API_KEY` is set** (free key; module returns null and the Market Wire cards just don't render). Rides on `/api/market/wire` as `fred`. Tests: `tests/fred-series.test.js`.
