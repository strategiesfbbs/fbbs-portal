...
- **Market Color news hub:** the `#market-color` tab is now a Bloomberg-style hub — live Market Wire indicator cards + Fed/FDIC/SEC headlines/auctions rail (reuses `/api/market/wire` and the Home wire card markup), and the desk's `.eml` market-color emails as articles with a lead card, tag-filter chips, search, and an in-page reading pane (`GET /api/market-color/files/:id/body` re-parses the stored `.eml` via `emailSummary`; raw `.eml` download unchanged). Wire and inbox degrade independently. Inbox ingest dedups by .eml content hash (same-day re-publishes resend the same batch; a startup pass cleaned the pre-dedup duplicates); previews strip safe-link URLs at render.
