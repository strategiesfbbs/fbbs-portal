...
- **Tear-sheet Sales Workspace split:** the bank tear sheet is two tabs under the header (`bankTearSheetTab`, session-persistent): **Sales Workspace** (default — signals strip, Today's Fits/Strategy Request actions, Activity/Tasks/Opportunities/Contacts/Sales Assistant/Product Fit/Services/Strategy History) and **Call Report & Portfolio** (the untouched call-report sections + FDIC bar + intelligence panel + uploaded files + Peer Report/Portfolio Review buttons). The signals strip (`buildBankSignalChips`) shows last-touch recency (30/60d bands), overdue/next task, open pipeline, this bank's CDs rolling ≤180d (`GET /api/banks/:id/cd-rollover`, a per-bank slice of the rollover universe), and FDIC freshness; it re-renders from every activity/task/opp panel refresh. Insight→action: Today's Fits picks have "+ Opp" (creates a `Securities Purchase` opportunity) and "Log pitch" (timeline note); the Sales Assistant call note has "Log as activity". `@media print` shows both tab panels so Print is unchanged.
