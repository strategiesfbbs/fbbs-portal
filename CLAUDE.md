...
- `server/strategy-store.js` — SQLite-backed Strategies Queue for Bond Swap, Muni BCIS, CECL Analysis, and Miscellaneous requests. Workflow statuses are `Open`, `In Progress`, `Completed`, and `Needs Billed`; requests can be created from a bank tear sheet and worked from the Strategies tab.
