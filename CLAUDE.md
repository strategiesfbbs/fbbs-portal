...
- `server/fred-series.js` — FRED rate benchmarks (2026-06-11): SOFR / effective fed funds / 10Y breakeven, 6h TTL, per-series outage keeps that series' cached print. **Dormant until a key is configured** — `FRED_API_KEY` env var, or a one-line `data/market/fred-api-key.txt` (gitignored; read at startup so the launchers need no env setup). Configured + live on this box since 2026-06-12. Rides on `/api/market/wire` as `fred`. Tests: `tests/fred-series.test.js`.
