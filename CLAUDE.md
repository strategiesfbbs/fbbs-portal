...
- **Open: 1 parser fix for Codex (L14)** — from the 2026-06-03 review. All 10 mediums + 9 safe lows + five of the six parser-behavior lows are fixed on origin/main (L15 agencies/corporates header scan, L16 treasury `findAsOfDate`, L17 bank-importer sheet9 fallback, L19 internal-CD misclassify, L20 stale-package merge — all verified resolved in code 2026-06-09). The one still open is **L14 econ-update date pairing**: `parseEconomicEvents()` in `server/economic-update-parser.js` collects `pendingDates[]` but never zips them to `pendingNames[]`, so every non-inline release renders dateless (`dateTime: null`) even when the counts match — needs a real econ-update PDF as a fixture to fix safely. Full spec in `docs/go-live/codex-handoff-parser-lows-2026-06-03.md`.
