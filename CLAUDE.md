...
**Portfolio Idea Engine (the Bond Swap home for bond-accounting banks).** `GET /api/swap-proposals/suggested?bankId=` is now a server-side port of the standalone "FBBS Portfolio Idea Engine" prototype — it runs the desk's Portfolio Filtering workflow off the already-parsed bond-accounting holdings + the workbook's cashflow series, and returns a full report. The response carries `kept` (the screened swap candidates, full list), `dropped` (hard-rule failures), `knobs`, `profile`, `runoff`, `hero`, and `findings` (server-composed prose cards). The UI ([portal.js](public/js/portal.js)) renders a knobs bar, an Opportunity Summary hero, a portfolio snapshot, sector bars, a maturity/runoff table, the top-12 candidate cards, an interactive selectable **blotter** with live package totals + CSV export, and the narrative findings.

- **Knobs** (all optional query params; defaults shown): `taxRate` (from the bank's Sub-S election — 21% C-corp / 29.6% Sub-S), `cof` (1.5), `bq` (0.20 BQ / 1.00 non-BQ), `reinvestRate` (target YTW; blank = auto), `maxPctLoss` (4.0), `maxDollarLoss` (10, $000), `minPar` (100, $000), plus `minPickup` / `smallGlPct`.
- **TEY for exempt munis.** Held exempt-muni yields are gross-up'd to a taxable-equivalent basis via `swapMath.municipalTeYield()` — the verified FBBS form `(YTW − COF·t·q)/(1−t)` — *before* comparing to the reinvestment target, so munis screen like-for-like against taxable reinvestment.
- **Entry screen is a UNION** (deliberately inclusive): a position qualifies if **any** of — (1) **below reinvestment** (its effective yield, TEY for munis, sits under the target minus a 10bp margin); (2) **cheap to move** (|G/L| ≤ `smallGlPct`% of book); (3) **loss harvest** (market yield ≥ 1% above book) — *and* it clears the loss-budget gates: any realized loss within `maxPctLoss`% and `maxDollarLoss`$, position ≥ `minPar`. Each candidate carries `tags[]` (`below-reinvest`/`small-gain`/`small-loss`/`loss-harvest`/`yield-pickup`), `held.{effYield,teYield,gainLossPct,isExemptMuni,wal,effDuration}`, `pickupVsReinvest`, `addedAnnualIncome`, and `reinvestBreakevenYears` (= |%loss|/annual pickup).
- **Reinvestment target** = a single rate. If `reinvestRate` is passed it's used everywhere; otherwise it's auto-derived as the best attainable taxable yield across today's buy universe (agency/treasury/corporate/CD, top-decile). Returned as `reinvestTarget` + `reinvestTargetSource`.
- **Runoff pipeline** comes from the workbook's `cashflow data` sheet (parsed by [portfolio-parser.js](server/portfolio-parser.js) into `parsedHoldings.cashflow`, schema v3): projected base-balance decline at 6/12/24mo (calls + amortization + maturities) vs. stated-maturity runoff.
- **Matched buy vs. generic reinvest.** Each candidate prefers a concrete same-sector buy from today's inventory that beats the held book yield (`yieldPickupVsBook`, `offering.cusip` set) so "Build proposal from this" seeds a real two-leg draft. When no offering beats it, a *below-reinvest* position still surfaces with a **generic "reinvest at target"** buy (`offering.generic = true`, no CUSIP) — this is what surfaces munis, whose nominal yield few muni offerings beat even though their TE yield is under the taxable target. The hard rule (held maturity ≥ breakeven) is still the only thing that *drops* a candidate (into `dropped[]`).
...
GET    /api/swap-proposals/suggested?bankId=X  Portfolio Idea Engine → { kept, dropped, knobs, profile, runoff, hero, findings, reinvestTarget } (opt knobs: taxRate, cof, bq, reinvestRate, maxPctLoss, maxDollarLoss, minPar)
